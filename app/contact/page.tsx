import { pageMeta } from "@/lib/page-metadata";
import { Breadcrumbs } from "@/components/breadcrumbs";

export const metadata = pageMeta({
  title: "Контакти - SEO BAZA",
  description: "Зв'яжіться з нами через Telegram або email info@seobaza.com.ua",
  path: "/contact",
});

export default function ContactPage() {
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="max-w-2xl mx-auto">
        <Breadcrumbs items={[{ name: "Головна", href: "/" }, { name: "Контакти", href: "/contact" }]} />
        <h1 className="text-4xl sm:text-5xl font-display mb-8 text-center bg-gradient-to-r from-accent to-primary bg-clip-text text-transparent">
          Контакти SEO BAZA
        </h1>

        <div className="bg-secondary/30 rounded-2xl p-8 border border-border text-center">
          <h2 className="text-2xl font-display mb-2">
            Події, партнерства та спонсорство
          </h2>
          <p className="text-muted-foreground mb-6">
            Хочете додати свою подію в наш календар, стати партнером чи спонсором? Пишіть Bronso.{" "}
            <a href="/sponsors" className="text-primary hover:text-accent underline transition-colors">
              Формати й ціни
            </a>
          </p>
          <a
            href="https://telegram.me/TheBronso"
            target="_blank"
            className="inline-flex items-center gap-3 text-2xl font-bold text-primary hover:text-accent transition-colors"
          >
            <svg
              className="w-8 h-8"
              fill="currentColor"
              viewBox="0 0 24 24"
            >
              <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
            </svg>
            @TheBronso
          </a>
        </div>

        <div className="bg-secondary/30 rounded-2xl p-8 border border-border text-center mt-6">
          <h2 className="text-2xl font-display mb-2">
            Усі інші питання
          </h2>
          <p className="text-muted-foreground mb-6">
            Контент, співпраця, загальні питання
          </p>
          <a
            href="https://telegram.me/fajela"
            target="_blank"
            className="inline-flex items-center gap-3 text-2xl font-bold text-primary hover:text-accent transition-colors"
          >
            <svg
              className="w-8 h-8"
              fill="currentColor"
              viewBox="0 0 24 24"
            >
              <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
            </svg>
            @fajela
          </a>
        </div>

        <div className="bg-secondary/30 rounded-2xl p-8 border border-border text-center mt-6">
          <h2 className="text-2xl font-display mb-2">
            Email
          </h2>
          <p className="text-muted-foreground mb-6">
            Загальні питання, конфіденційність, виправлення
          </p>
          <a
            href="mailto:info@seobaza.com.ua"
            className="inline-flex items-center gap-3 text-2xl font-bold text-primary hover:text-accent transition-colors break-all"
          >
            <svg
              className="w-8 h-8 shrink-0"
              fill="none"
              stroke="currentColor"
              strokeWidth={2}
              viewBox="0 0 24 24"
            >
              <path strokeLinecap="round" strokeLinejoin="round" d="M3 8l9 6 9-6M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
            </svg>
            info@seobaza.com.ua
          </a>
        </div>

      </div>
    </div>
  );
}
